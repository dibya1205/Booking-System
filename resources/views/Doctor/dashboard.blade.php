...
<br>
  <a style="color: white;" class="btn btn-primary" href="{{route('doctor.dashboard')}}">
  Appointments
</a>
<br>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Doctors | Profile</h5>
      ...
      <div class="modal-body">
        ...
        Specialization: @php print_r(Auth::User()->spl); @endphp<br>
      </div> 
...
<b class="animate" >Kindly refresh the page after a successfull appointment</b>
<h2 style="text-align: center;" >Your Scheduled Appointments</h2><br>
    <table class="table table-hover table-dark" >
        <tr>
            <th><i class="fa fa-flag"></i>&nbsp; Sr. No</th>
            <th><i class="fa fa-heartbeat"></i>&nbsp;Patient's Name</th>
            <th><i class="fa fa-envelope"></i>&nbsp;Email Id</th>
            <th><i class="fa fa-phone"></i>&nbsp;Contact Number</th>
            <th><i class="fa fa-clock"></i>&nbsp;Timing</th>
            <th><i class="fas fa-calendar-check"></i>&nbsp;Appointment Status</th>
        </tr>
        @php
                ($appoint = DB::table('appointment')->where('department', Auth::User()->spl)->orderby('timing')->get());
        ...
        @foreach($appoint as $apt)
            <tr>
                ...
                <td>{{$apt->timing}}</td>
                <td>
                <form action="/dashboard/{{ $apt->id }}" method="POST">
                   {{ csrf_field() }}
                  {{ method_field('DELETE') }}
                  <button class="del">Delete Appointment</button>
                  <!-- <p>Click if the appointment is succesfull</p> -->
